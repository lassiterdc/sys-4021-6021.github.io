...
xdmgnd.pca <- prcomp(xdmgnd[-c(5900,5337,5901,5330),c("CARSDMG","EQPDMG", "TRKDMG","ACCDMG", "TOTKLD", "TOTINJ")])
...
barplot(xdmgnd.pca$rotation[,1], main='PC1 Loadings')
barplot(xdmgnd.pca$rotation[,2], main='PC2 Loadings')

# using ggplot
df = as.data.frame(xdmgnd.pca$rotation)
setDT(df, keep.rownames=TRUE)[]
names(df)[names(df) == "rn"] <- "Variable"
PC1 <- ggplot(data=df, aes(x=Variable,y=PC1)) + geom_bar(stat="identity") + ggtitle("PC1 Loadings with Covariance Matrix")
PC2 <- ggplot(data=df, aes(x=Variable,y=PC2)) + geom_bar(stat="identity") + ggtitle("PC2 Loadings with Covariance Matrix")
...
par(mfrow=c(2,2))
barplot(xdmgnd.pca$rotation[,1], main='PC1 Loadings with Covariance Matrix')
barplot(xdmgnd.pca.corr$rotation[,1], main='PC1 Loadings with Correlation Matrix')
barplot(xdmgnd.pca$rotation[,2], main='PC2 Loadings with Covariance Matrix')
barplot(xdmgnd.pca.corr$rotation[,2], main='PC2 Loadings with Correlation Matrix')
par(mfrow=c(1,1))

# with ggplot
df_cov = as.data.frame(xdmgnd.pca$rotation) # get first 6 columns which are the 6 PCs
setDT(df_cov, keep.rownames=TRUE)[]
names(df_cov)[names(df_cov) == "rn"] <- "Variable"
PC1 <- ggplot(data=df_cov, aes(x=Variable,y=PC1)) + geom_bar(stat="identity") + ggtitle("PC1 Loadings with Covariance Matrix")
PC2 <- ggplot(data=df_cov, aes(x=Variable,y=PC2)) + geom_bar(stat="identity") + ggtitle("PC2 Loadings with Covariance Matrix")

df_corr = as.data.frame(xdmgnd.pca.corr$rotation) # get first 6 columns which are the 6 PCs
setDT(df_corr, keep.rownames=TRUE)[]
names(df_corr)[names(df_corr) == "rn"] <- "Variable"
PC1_corr <- ggplot(data=df_corr, aes(x=Variable,y=PC1)) + geom_bar(stat="identity") + ggtitle("PC1 Loadings with Correlation Matrix")
PC2_corr <- ggplot(data=df_corr, aes(x=Variable,y=PC2)) + geom_bar(stat="identity") + ggtitle("PC2 Loadings with Correlation Matrix")
...
pred.pca <- prcomp(xdmgnd[-c(5337),c("ACCDMG", "TRNSPD", "CARS", "TIMEHR", "TEMP")], scale = T )
